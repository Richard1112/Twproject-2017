...
<%!
  private void drawParent(RemoteFile parentFile, PageSeed ps, PageContext pc, PageState pst, String rootPath) throws IOException {
    ...
    ps.addClientEntry("TASK_ID", pst.getEntry("TASK_ID").stringValueNullIfEmpty());
    ps.setPopup(pst.isPopup());
    String m=parentFile.getRelativePath();
    String m2=rootPath;
    ButtonLink up = new ButtonLink("PARENT_FOLDER", ps);
    if(parentFile.getRelativePath().length()>1){
        ...
        <td>
          <table>
            ...
            %> <td><%
              ...
              %></td><td><%
                PageSeed p = TaskReportBricks.getPageSeedForExplorer(request, pageState, remoteFile, exp.document);
...
<td valign="top" width="70%">
  ...
  <table class="table" border="0"><%
    ...
    for (RemoteFile remoteFile : files) {
      if (!remoteFile.isDirectory()) {
        jsonFiles.add(remoteFile.jsonifyReport());
        howManyFiles++;
...
<div id="rowTemplates" style="display: none;">
  ...
  <table border=0>
    ...
    %></td><td>
      ...
      <td>&nbsp;<small>(#=obj.uploadUser#)</small>)</td>
    ...
    $(function () {
      ...
      if(<%=exp.canWrite?"true":"false"%>){
        $("#dropFilesHere").uploadize({
          url:"<%=ApplicationState.contextPath%>/applications/teamwork/task/report/reportExplorerAjaxController.jsp",
          fieldName:"UPLOAD_HERE",
          preview:false,
          fileAreaSelector:"#boxFiles",
          additionalRequestParameters:{CM:"UPLOAD",OBJID:"<%=pageState.mainObjectId%>","TASK_ID":<%=pageState.getEntry("TASK_ID").stringValueNullIfEmpty()%>,PATH:"<%=rfs.getRelativePath()%>"},
