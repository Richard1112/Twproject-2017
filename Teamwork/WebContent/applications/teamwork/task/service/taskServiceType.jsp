<%@page import="com.teamwork.expand.TaskServiceType"%>
  ...
  if (!pageState.screenRunning) {
    ...
    if (Commands.SAVE.equals(pageState.command)) {
      ...
      for (String id : map.keySet()) {
    	...
    	TaskServiceType t = (TaskServiceType) PersistenceHome.findByPrimaryKey(TaskServiceType.class, id);
      ...
      if (newDesc != null) {
    	TaskServiceType t = new TaskServiceType();
    ...
    String hql = "from " + TaskServiceType.class.getName() + " as tt ";
    OqlQuery oql = new OqlQuery(hql);
    List<TaskServiceType> tts = oql.list();

%><h1><%=I18n.get("SERVICE_TYPE_MENU")%></h1><%
...
%><table class="table"><tr><th class="tableHead">Id</th><th class="tableHead"><%=I18n.get("SERVICETYPE")%></th><th class="tableHead"><%=I18n.get("DESCRIPTION")%></th><th class="tableHead"><%=I18n.get("DELETE_SHORT")%></th></tr><%


    for (TaskServiceType tt : tts) {
...
%>
  ...
  %><td colspan="2"><%tf.toHtml(pageContext);%></td></tr><%
    ...
    new DeletePreviewer("OBJ_DEL", TaskServiceType.class.getName(), pageState);
