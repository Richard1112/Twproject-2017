<%@ page import="com.twproject.resource.Resource,
                 com.twproject.resource.businessLogic.ResourceAction,
...
<%!
  ...
  public class AlternativeResourceList extends PagePlugin {
    ...
    public boolean isVisibleInThisContext(PageState pagestate) {
      boolean ret = pagestate.href.indexOf("resourceList") >= 0 && JSP.ex(pagestate.getPage());
      return ret;
    }
...
%><%
  ...
  if (JspIncluder.INITIALIZE.equals(request.getParameter(Commands.COMMAND))) {
    ...
    if (!pageState.screenRunning) {
      ...
      %><h2>Resource custom  list</h2><%
      List<Resource> rs = pageState.getPage().getAllElements();
      for (Resource r : rs) {
        %><%=r.getDisplayName()%><br><%
      }
