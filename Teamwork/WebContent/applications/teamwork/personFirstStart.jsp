<%@ page
        import="com.twproject.operator.TeamworkOperator, com.twproject.resource.Person, com.twproject.waf.TeamworkHBFScreen, org.jblooming.anagraphicalData.AnagraphicalData, org.jblooming.http.multipartfilter.MultipartFormRequestEncodingFilter, org.jblooming.ontology.PersistentFile, org.jblooming.utilities.JSP, org.jblooming.utilities.file.FileUtilities, org.jblooming.waf.ScreenArea, org.jblooming.waf.constants.Commands, org.jblooming.waf.constants.OperatorConstants, org.jblooming.waf.html.button.ButtonSubmit, org.jblooming.waf.html.container.ButtonBar, org.jblooming.waf.html.display.Img, org.jblooming.waf.html.input.TextField, org.jblooming.waf.html.input.Uploader, org.jblooming.waf.html.state.Form, org.jblooming.waf.settings.I18n, org.jblooming.waf.view.PageSeed, org.jblooming.waf.view.PageState, java.io.File" %>
<%
  ...
  if (!pageState.screenRunning) {
    ...
    if (Commands.SAVE.equals(pageState.command)) {
      ...
      String imageUrl = pageState.getEntry("uploadedImgUrl").stringValueNullIfEmpty();
      ...
      if (JSP.ex(email)){
        if(person.getAnagraphicalDatas().size() > 0){
           for (AnagraphicalData a :person.getAnagraphicalDatas()){
             a.setEmail(email);
             break;
           };
        }else{
      ...
      if (pageState.validEntries()) {
        ...
        response.sendRedirect(com.twproject.setup.WizardSupport.getHomePage(pageState));
        return;
      }
    ...
    Img myphoto = person.bricks.getAvatarImage("30");

    if(myphoto !=null){
      myphoto.id = "personalAvatar";
    }

    TextField tf = new org.jblooming.waf.html.input.TextField(I18n.get(org.jblooming.waf.constants.OperatorConstants.FLD_NAME), org.jblooming.waf.constants.OperatorConstants.FLD_NAME, "", 30, false);
    tf.fieldClass = "formElements formElementsBig";

    org.jblooming.waf.html.input.TextField tfs = new org.jblooming.waf.html.input.TextField(I18n.get(org.jblooming.waf.constants.OperatorConstants.FLD_SURNAME), org.jblooming.waf.constants.OperatorConstants.FLD_SURNAME, "", 30, false);
    tfs.fieldClass = "formElements formElementsBig";

      org.jblooming.waf.html.input.TextField email = new org.jblooming.waf.html.input.TextField(I18n.get("EMAIL"), "EMAIL", "", 30, false);
...
<%
  ...
  %>
<%--------------------------------------------------------------------------   ADDITIONAL REQUIRED DATA FROR AVATAR    ---------------------------------------------------------------------------------------------------------%>
...
<script type="text/javascript">
  ...
  function loadImage(el) {
    var url = "";
    var type = el.closest("[imageType]").attr("imageType");
    var addData = el.closest("[imageType]").find(":input").val();
    if(addData){
      var setAvatarImg = function(url){
        $("#personalAvatar").prop("src",url);
        $("[name=uploadedImgUrl]").val(url);
      }
      if (type == "facebook") {
        setAvatarImg("https://graph.facebook.com/"+addData+"/picture");

      } else if (type == "gravatar") {
        setAvatarImg(get_gravatar(addData));
      } else if (type == "twitter") {
        getTwitterImg(addData, setAvatarImg)
      }
    }

    el.closest("[imageType]").fadeOut();
  }
  ...
  function get_gravatar(email) {
    ...
    return 'https://www.gravatar.com/avatar/' + MD5(email) + '.jpg?s=80';
  }



  function getTwitterImg(userName, callback) {
    var url = "";
    $.ajax({
      url: "https://twitter.com/users/" + userName + ".json",
      dataType: 'jsonp',
      success: function(response) {
        callback(response.profile_image_url);
      }
    });
    return url;
  }
