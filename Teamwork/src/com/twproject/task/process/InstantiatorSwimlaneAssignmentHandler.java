...
public class InstantiatorSwimlaneAssignmentHandler extends DinamicAssignmentHandler {

  public Assignment getTeamworkAssignment(Assignable assignable, ExecutionContext executionContext) {
    Assignment assig = null;

    try {
      //resource is the twTaskRoot owner
      Operator owner = taskProcessRoot.getOwner();
      Resource twResource = ((TeamworkOperator) ReflectionUtilities.getUnderlyingObjectAsObject(owner)).getPerson();
      if (twResource != null) {
        ...
        throw new PlatformRuntimeException("Instantiator not found: in process " + executionContext.getProcessDefinition().getName() + " instance " + executionContext.getProcessInstance().getId());
      }
