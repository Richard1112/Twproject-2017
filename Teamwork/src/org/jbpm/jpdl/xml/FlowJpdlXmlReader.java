...
public class FlowJpdlXmlReader {
  ...
  public FlowJpdlXmlReader(Reader reader) {
    super(reader);
    this.reader=reader;
  }

  public FlowJpdlXmlReader(Reader reader, JbpmSession jbpmSession) {
    super(reader,jbpmSession);
    this.reader = reader;
  ...
  private void readSwimlanes(Element processDefinitionElement) {
    ...
    while (iter.hasNext()) {
      ...
      if (swimlaneName==null) {
        addMessageWarning("there's a swimlane without a name");
      } else {
        ...
        if (assignmentElement!=null) {
          ...
          if ( (startTask==null)
            ...
            addMessageWarning("swimlane '"+swimlaneName+"' does not have an assignment");
          }
        }
  ...
  public Task readTask(Element taskElement, TaskMgmtDefinition taskMgmtDefinition, TaskNode taskNode) {
    ...
    if (swimlaneName!=null) {
      ...
      if (swimlane==null) {
        addMessageError("task references unknown swimlane '"+swimlaneName+"':"+taskElement.asXML());
      } else {
    ...
    // if no assignment or swimlane is specified
      ...
      addMessageError("warning: no swimlane or assignment specified for task '"+taskElement.asXML()+"'");
    }
  ...
  public void resolveTransitionDestination(Element transitionElement, Node node) {
    ...
    if (name!=null) {
      ...
      addMessageWarning("node '"+node.getFullyQualifiedName()+"' has a transition without a 'name'-attribute");
    }
    ...
    if (toName==null) {
      addMessageError("node '"+node.getFullyQualifiedName()+"' has a transition without a 'to'-attribute to specify its destinationNode");
    } else {

      Node to= null;
      try {
        to = ((NodeCollection)node.getParent()).findNode(toName);
      } catch (Exception e) {
      }
      if (to==null) {
        addMessageError("transition to='"+toName+"' on node '"+node.getFullyQualifiedName()+"' cannot be resolved");
      } else {
  ...
  public void resolveActionReferences() {
    ...
    while (iter.hasNext()) {
      ...
      if (referencedAction==null) {
        addMessageError("couldn't resolve action reference in "+actionElement.asXML());
      }
    ...
    if ( (taskMgmtDefinition!=null)
      ...
      while (iter.hasNext()) {
        ...
        if ( (swimlane.getAssignmentDelegation()==null)
          ...
          addMessageWarning("swimlane '"+swimlane.getName()+"' does not have an assignment");
        }
      }
    }
  }
  ...
  public Collection getUnresolvedTransitionDestinations() {
    return unresolvedTransitionDestinations;
  }

  public Collection getUnresolvedActionReferences() {
    return unresolvedActionReferences;
  }
}*/
