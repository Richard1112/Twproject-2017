...
public class JSP extends StringUtilities {
   ...
   *
   * @param content
   * @param maxLength è il numero massimo di caratteri che la funzione ritorna, compresi i puntini
   * @return
   */
  ...
  public static String limWr(String content, int maxLength, boolean fromLeft) {
    if (JSP.ex(content)) {
      ...
      if (content.length() > maxLength) {
        if(fromLeft)
        content = content.substring(0, maxLength-dots.length()) + dots;
        else
          content = dots + content.substring(content.length()-maxLength-dots.length(), content.length());
      }
      return content;
    } else
      return "";
  ...
  public static final String javascriptEncode(String source) {
    if (source != null)
      return StringUtilities.replaceAllNoRegex(source, new String[]{"\\", "\"", "'", "’", "\n"}, new String[]{"\\\\", "\\\"", "\\'", "\\'", "\\n"});
    else
      return null;
  }
   ...
   *
   * @param contents
   * @return true when contents not null and content.length>0
   */
