 ...
 *  interesting (and efficient), the class follows a Flyweight pattern, so 
 *  that for each different N-Gram there will only be one instance in the System.
 *
...
public class NGram implements Comparable {
	 ...
	 *@param  str  A string with the N-gram.
	 */
	public NGram(String str, double count) {
		this(str.getBytes(), 0, str.getBytes().length, count);
	}
	...
	public double getSmoothedCount() {
		...
		if(numberCount==0) return 1 + count;
		return 1 + count * (numberCountPlusOne / numberCount);
	}
